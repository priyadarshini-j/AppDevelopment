...
//           <li><Link to="#" onClick={() => setActiveComponent('productform')}>Add Product</Link></li>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch profile data (assuming email is stored in localStorage or can be derived)
        const email = localStorage.getItem("userEmail"); // Ensure you have a method to get the email
        const profileResponse = await axios.get(`http://127.0.0.1:8080/api/users/readUser/${email}`, config);
        setProfile(profileResponse.data);
