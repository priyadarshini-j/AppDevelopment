...
//           "http://127.0.0.1:8080/api/users/readUsers", // Adjust the endpoint as per your backend API
//           config
//         );
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        const response = await axios.get(
          "http://127.0.0.1:8080/api/users/readUser", // Adjust the endpoint as per your backend API
          config
        );
        setProfileData(response.data); // Store the profile data, including the image URL
      } catch (error) {
  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        "http://127.0.0.1:8080/api/users/updateUser", // Adjust the endpoint as per your backend API
        profileData,
        config
      );
      if (response.status === 200) {
